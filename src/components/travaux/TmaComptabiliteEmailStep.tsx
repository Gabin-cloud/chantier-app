...
import {
  createTmaComptabiliteEmailDraft,
  prepareTmaComptabiliteEmailPreview,
  sendTmaComptabiliteEmail,
  type TmaComptabiliteEmailPreviewResult,
} from "@/lib/actions/tma-comptabilite-email";

type TmaComptabiliteEmailStepProps = {
  projectId: string;
  entryIds: string[];
  m365Ready: boolean;
  open: boolean;
...
export function TmaComptabiliteEmailStep({
  projectId,
  entryIds,
  m365Ready,
  open,
  onClose,
  onComplete,
}: TmaComptabiliteEmailStepProps) {
  const [, startTransition] = useTransition();
  const [loading, setLoading] = useState(true);
  const [loadError, setLoadError] = useState<string | null>(null);
  const [preview, setPreview] = useState<
    Extract<TmaComptabiliteEmailPreviewResult, { ok: true }> | null
  >(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      const result = await prepareTmaComptabiliteEmailPreview(projectId, entryIds);
      if (cancelled) return;
      if (!result.ok) {
  ...
  function submit(mode: "draft" | "send") {
    ...
    startTransition(async () => {
      const result =
        mode === "draft"
          ? await createTmaComptabiliteEmailDraft(projectId, entryIds, overrides)
          : await sendTmaComptabiliteEmail(projectId, entryIds, overrides);
      ...
      setSuccess(
        mode === "draft"
          ? "Brouillon Outlook créé — dépôts marqués envoyés à la comptabilité."
          : "Mail envoyé à la comptabilité."
      );
  ...
  return (
    <ModalPanel
      ...
      subtitle="Dépôts TMA analysés — PDF renommés automatiquement"
      onClose={onClose}
      maxWidth="xl"
    >
      {loading ? (
