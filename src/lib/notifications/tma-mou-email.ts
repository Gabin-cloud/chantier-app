...
export const DEFAULT_TMA_MOU_EMAIL_SUBJECT =
  "{{project_name}} — TMA logement {{logement_numbers}}";

export const DEFAULT_TMA_MOU_EMAIL_BODY = `<p>Bonjour,</p><p>Veuillez trouver ci-joint les devis TMA pour l'opération <strong>{{project_name}}</strong> (logements : {{logement_numbers}}).</p><p>Total H.T. : <strong>{{total_ht}}</strong></p><p>Cordialement,<br/>DANOBAT</p>`;

export const TMA_MOU_EMAIL_MERGE_TAGS: MergeTagDefinition[] = [
  {
    key: "project_name",
  ...
  },
  {
    ...
    description: "Nombre de PDF joints",
    example: "3",
  },
  {
    key: "total_ht",
...
export type TmaMouEmailDraftInput = {
  projectName: string;
...
export function buildTmaMouEmailFromTemplates(
  subjectTemplate: string,
  bodyTemplate: string,
  input: TmaMouEmailDraftInput
